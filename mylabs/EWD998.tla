...
X0) Sending a message by node  i  increments a counter at  i  , the receipt of a
   message decrements i's counter
X3) Receiving a *message* (not token) blackens the (receiver) node

X2) An active node j -owning the token- keeps the token.  When j becomes inactive,
   it passes the token to its neighbor with  q = q + counter[j] 

X4) A black node taints the token

X7) Passing the token whitens the sender node

X1) The initiator sends the token with a counter  q  initialized to  0  and color
   white

X5) The initiator starts a new round iff the current round is inconclusive
   Round is "conclusive" iif TokenColor is White AND TokenCounter is 0

X6) The initiator whitens itself and the token when initiating a new round
